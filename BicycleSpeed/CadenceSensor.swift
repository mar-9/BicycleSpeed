...
//    static let CadenceService         = "1818"
    static let CadenceService         = "1816"  // Cycling Speed and Cadence
...
class CadenceSensor: NSObject, CBPeripheralDelegate {
    ...
    func handleValueData( data:NSData ) {
        ...
        sensorDelegate?.sensorUpdatedValues(
            speedInMetersPerSecond: values?.speedInMetersPerSecond,
            cadenceInRpm: values?.cadenceinRPM,
    ...
    func peripheral(
        ...
        guard error == nil else {
            sensorDelegate?.errorDiscoveringSensorInformation(
                error: NSError(
                    domain: CBErrorDomain,
                    code: 0,
                    userInfo: [NSLocalizedDescriptionKey:NSLocalizedString("Error receiving measurements updates", comment:"")]
                )
            )
            return
        }
    ...
    func peripheral(
        ...
        handleValueData(data: data as NSData)
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        guard error == nil else {
            sensorDelegate?.errorDiscoveringSensorInformation(error: NSError(
                    domain: CBErrorDomain,
                    code: NSNotFound,
                    userInfo: [NSLocalizedDescriptionKey:NSLocalizedString("didDiscoverServices", comment:error!.localizedDescription )]
                )
            )
            return
        }
        ...
        guard let cadenceService =  peripheral.services?.filter({ (service) -> Bool in
            ...
            sensorDelegate?.errorDiscoveringSensorInformation(
                error: NSError(
                    domain: CBErrorDomain,
                    code: NSNotFound,
                    userInfo: [NSLocalizedDescriptionKey:NSLocalizedString("Cadence service not found for this peripheral", comment:"")]
                )
            )
            return
        }
        ...
        guard let characteristics = service.characteristics else {
            sensorDelegate?.errorDiscoveringSensorInformation(
                error: NSError(
                    domain: CBErrorDomain,
                    code: NSNotFound,
                    userInfo: [NSLocalizedDescriptionKey:NSLocalizedString("No characteristics found for the cadence service", comment:"")]
                )
            )
            return
        }
